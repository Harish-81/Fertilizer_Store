...
const Admin = () => {
  ...
  const allowedAdminEmails = ["anashenrya.22aim@kongu.edu","harishsv.22aim@kongu.edu"]; // List of admin-approved emails
  ...
  const generatePDFReport = () => {
    ...
    orders.forEach((order) => {
      ...
      const itemsList = order.orderItems
        .map((item) => {
          const productName = item.product ? item.product.name : "Deleted";
              ...
              {orders.map((order) => (
                <li key={order._id} className='admin-order-item'>
                  ...
                  <p>
                    <strong>Paid:</strong>{" "}
                    {order.isPaid
                      ? `Yes (at ${new Date(order.createdAt).toLocaleString()})`
                      : "No"}
                  </p>
                  <div>
                    <strong>Items:</strong>
